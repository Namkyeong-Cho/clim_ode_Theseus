...
#import warnings
from conv_model import DualInputConvNet
...
class ClimODE_uncertain_region(nn.Module): 
    ...
    def pde(self,t,vs):
        ...
        if self.pos:
            ...
            pos_feats = torch.cat([cos_lat_map,cos_lon_map,sin_lat_map,sin_lon_map,sin_lat_map*cos_lon_map,sin_lat_map*sin_lon_map],dim=1)
            pos_time_ft = self.get_time_pos_embedding(t_cyc_emb,pos_feats)
            comb_rep = torch.cat([t_emb/24,day_emb,seas_emb,
                                  nabla_u,v,ds,self.new_lat_map,self.new_lon_map,self.lsm,self.oro,pos_feats,pos_time_ft],dim=1)
...
class Climate_encoder_free_uncertain_monthly(nn.Module): 
    ...
    def __init__(self,num_channels,const_channels,out_types,method,use_att,use_err,use_pos):
        ...
        self.vel_f = Climate_ResNet_2D(input_channels,self.layers,self.hidden)
        # device = next(self.vel_attdel.parameters()).device
        self.diffusive_terms =nn.Parameter(torch.full((1, 5, 32, 64), 0.1, requires_grad=True)).cuda()
        # print("device: " , device)
        # assert False
        self.dual_conv_net = DualInputConvNet()
        if use_att: 
            self.vel_att = Self_attn_conv(input_channels,10)
            self.gamma = nn.Parameter(torch.tensor([0.1]))
    ...
    def pde(self,t,vs):
        ...
        # print("vs : " , vs.shape)
        # print("sin_t_emb  : " , sin_t_emb.shape)
        # print("sin_t_emb  : " , cos_t_emb.shape)
        # print("t  : " , t)
        ...
        dv_grad_x = torch.gradient(v,dim=3)[0]
        dv_grad_y = torch.gradient(v,dim=2)[0]
        
        nabla_v = torch.cat([dv_grad_x,dv_grad_y],dim=1)
        nabla_u = torch.cat([ds_grad_x,ds_grad_y],dim=1)

        # print("ds        : ", ds.shape)
        # print("ds_grad_x : ", ds_grad_x.shape)
        # print("ds_grad_y : ", ds_grad_y.shape)
        # print("nabla_u   : ", nabla_u.shape)
        ds_grad_xx = torch.gradient(ds_grad_x,dim=3)[0]
        ds_grad_yy = torch.gradient(ds_grad_y,dim=2)[0]
        Delta_u = ds_grad_xx + ds_grad_yy
        
        
        div_v = dv_grad_x + dv_grad_y
        div_u = ds_grad_x + ds_grad_y
        # print(" self.pos : ", self.pos)
        # print("div_u shape: " , div_u.shape)
        # print("div_v shape: " , div_v.shape)
        if self.pos:
            # comb_rep = torch.cat([t_emb/6,day_emb,nabla_u,v,ds,self.pos_feat],dim=1)
            # comb_rep = torch.cat([t_emb/6,day_emb,nabla_u, v, nabla_v, ds,self.pos_feat],dim=1)
            comb_rep = torch.cat([t_emb/6,day_emb,nabla_u, Delta_u, v,nabla_v, ds, self.pos_feat],dim=1)
            ...
            pos_feats = torch.cat([cos_lat_map,cos_lon_map,sin_lat_map,sin_lon_map,sin_lat_map*cos_lon_map,sin_lat_map*sin_lon_map],dim=1)
            pos_time_ft = self.get_time_pos_embedding(t_cyc_emb,pos_feats)
            # comb_rep = torch.cat([t_emb/6,day_emb,nabla_u, v,ds,
            #                       self.new_lat_map,self.new_lon_map,self.lsm,self.oro,pos_feats,pos_time_ft],dim=1)
            # comb_rep = torch.cat([t_emb/6,day_emb,nabla_u, v, nabla_v, ds,
            #                       self.new_lat_map,self.new_lon_map,self.lsm,self.oro,pos_feats,pos_time_ft],dim=1)
            comb_rep = torch.cat([t_emb/6,day_emb,nabla_u, v, nabla_v, Delta_u, div_u, div_v,  ds,
                                  self.new_lat_map,self.new_lon_map,self.lsm,self.oro,pos_feats,pos_time_ft],dim=1)
            # comb_rep = torch.cat([t_emb/6,day_emb,nabla_u, v, nabla_v, Delta_u,  ds,
            #                       self.new_lat_map,self.new_lon_map,self.lsm,self.oro,pos_feats,pos_time_ft],dim=1)
        
        # print("To add diffusion term, dimesion check is necessary!")
        # print("#"*100)
        # print("t_emb : ",t_emb.shape)
        # print("day_emb : ",day_emb.shape)
        # print("nabla_u : ",nabla_u.shape)
        # print("v       : ",v.shape)
        # print("ds      : ",ds.shape)
        # print("self.new_lat_map : ",self.new_lat_map.shape)
        # print("self.new_lon_map : ", self.new_lon_map.shape)
        # print("self.lsm         : ", self.lsm.shape)
        # print("self.oro         : ", self.oro.shape)
        # print("pos_feats        : ", pos_feats.shape)
        # print("pos_time_ft      : ", pos_time_ft.shape)
        # print("#"*100)
        # print("comb_rep : " , comb_rep.shape)
        ...
        # print("v_y : " , v_y.shape)
        # print("ds        : ", ds.shape)
        # print("ds_grad_xx : ", ds_grad_xx.shape)
        # print("ds_grad_yy : ", ds_grad_yy.shape)
        # print("Delta_u       : ", Delta_u.shape)
        # print(self.diffusive_terms.shape)
        # print("self.pos  : ", self.pos)
        ...
        # print("adv1      : ", adv1.shape)
        # print("avd2      : ", adv2.shape)
        # print("Delta_u   : ", Delta_u.shape)
        # print("self.pos  : ", self.pos)
        # assert False 

        B=Delta_u.shape[0]
        # ds = adv1 + adv2 +  self.dual_conv_net(self.diffusive_terms.expand(B, -1, -1, -1)
        relu = torch.nn.ReLU()                   
        Delta_u.to(self.diffusive_terms.device)
        self.diffusive_terms = self.diffusive_terms.to('cuda:0')
        # print("device check 1:", Delta_u.device)
        # print("device check 2:", self.diffusive_terms.device)
        # print("device check 2:", self.diffusive_terms.device)
        # print("device check 3:", adv1.device)
        # print("device check 4:", adv2.device)
        ds = adv1 + adv2 +  relu(self.diffusive_terms.expand(B, -1, -1, -1))* Delta_u 
