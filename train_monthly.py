...
parser.add_argument('--name_of_model', type=str, default="default")
...
const_info_path = ['~/ClimODE/era5_data/constants/constants_5.625deg.nc']
...
for idx,data in enumerate(paths_to_data):
    Train_data,Val_data,Test_data,time_steps,lat,lon,mean,std,time_stamp \
        = get_train_test_data_without_scales_batched_monthly(data,train_time_scale,val_time_scale,test_time_scale,levels[idx],args.spectral)  
    max_lev.append(mean)
    min_lev.append(std)
    if idx==0: 
    ...
    # print(f"{idx}: Final_train_data : ", Final_train_data.shape)
    # print(f"{idx}: Final_val_data : ", Final_val_data.shape)
    # print(f"{idx}: Final_test_data : ", Final_test_data.shape)
...
fit_velocity(time_idx,time_loader,Final_train_data,Train_loader, torch.device('cpu'),
    num_years,paths_to_data,args.scale,H,W,types='train_monthly',
    vel_model=Optim_velocity,kernel=kernel,lat=lat,lon=lon)

fit_velocity(time_idx,time_loader,Final_val_data,Val_loader,torch.device('cpu'),
             1,paths_to_data,args.scale,H,W,types='val_monthly',
             vel_model=Optim_velocity,kernel=kernel,lat=lat,lon=lon)

fit_velocity(time_idx,time_loader,Final_test_data,Test_loader,torch.device('cpu'),
             2,paths_to_data,args.scale,H,W,types='test_monthly',
             vel_model=Optim_velocity,kernel=kernel,lat=lat,lon=lon)
...
for epoch in range(args.niters):
    ...
    if val_loss < best_loss:
        best_loss = val_loss
        best_epoch = epoch
        torch.save(model,str(cwd) + "/Models/" + "ClimODE_monthly_"
                +str(args.name_of_model)+"_"+args.solver+"_"+str(args.spectral)+"_model_" + str(epoch) + ".pt")
