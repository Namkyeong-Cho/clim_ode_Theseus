...
def nll(mean,std,truth,lat,var_coeff):
    ...
    # print(mean.shape)
    u_x = torch.gradient(mean,dim=3)[0][:,:,3,:,:]
    u_y = torch.gradient(mean,dim=4)[0][:,:,4,:,:]

    # # print(torch.gradient(mean,dim=4).shape)
    # print("u_x : ", u_x.shape)
    # print("u_y : ", u_y.shape)
    # assert False
    
    # divergence_free_loss  =  torch.sum(u_x + u_y)
    # loss_val += divergence_free_loss
    return loss_val
...
def fit_velocity(time_idx,time_loader,Final_train_data,data_loader,
                 device,num_years,paths_to_data,scale,H,W,types,
                 vel_model,kernel,lat,lon):
    num =0
    cwd = os.getcwd() 
    for idx_steps,time_steps,batch in zip(time_idx,time_loader,data_loader):
        ...
        v_x,v_y,loss_terms,out = optimize_vel(num_years,data,delta_u,vel_model,kernel,H,W,steps=200)
