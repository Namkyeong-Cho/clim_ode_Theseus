...
for idx,data in enumerate(paths_to_data):
    Train_data,Val_data,Test_data,time_steps,lat,lon,mean,std,time_stamp = get_train_test_data_without_scales_batched_monthly(data, train_time_scale, 
            val_time_scale, test_time_scale, 
            levels[idx], args.spectral)  
    max_lev.append(mean)
    min_lev.append(std)
    if idx==0: 
...
# ClimODE_monthly_avg_nabla_v_add_diff_euler_0_model_1990.pt
# model_path = '/Models/ClimODE_monthly_avg_euler_0_model_1561.pt'
# model_path = '/Models/ClimODE_monthly_avg_nabla_v_euler_0_model_1977.pt'
# model_path = '/Models/ClimODE_monthly_avg_Delta_euler_0_model_1995.pt'
# model_path = '/Models/ClimODE_monthly_avg_nabla_v_add_diff_euler_0_model_1977.pt'
# model_path = '/Models/ClimODE_monthly_mlp_diff_euler_0_model_1996.pt'
# model_path ='/Models/ClimODE_monthly_div_free_pinn_loss_euler_0_model_770.pt'

model_path = '/Models/ClimODE_monthly_relu_only_euler_0_model_1993.pt'
model_path = '/Models/ClimODE_monthly_divergence_feature_euler_0_model_1996.pt'
model = torch.load(str(cwd) + model_path,map_location=torch.device('cpu')).to(device)
# print(model)
# model = torch.load(str(cwd) + "./Models",map_location=torch.device('cpu')).to(device)
print(model)
...
for entry,(time_steps,batch) in enumerate(zip(time_loader,Test_loader)):
        ...
        for yr in range(3): 
            for t_step in range(1,len(time_steps),1):
                evaluate_rmsd = evaluation_rmsd_mm(mean_pred[t_step,yr,:,:,:].cpu(),
                                                   batch[t_step,yr,:,:,:].cpu(),lat,lon,max_lev,min_lev,H,W,levels)
                evaluate_acc = evaluation_acc_mm(mean_pred[t_step,yr,:,:,:].cpu(),
                                                 batch[t_step,yr,:,:,:].cpu(),lat,lon,max_lev,min_lev,H,W,levels,clim[yr,:,:,:].cpu().detach().numpy())
